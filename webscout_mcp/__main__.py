...
async def _cmd_serve(args: argparse.Namespace) -> None:
    ...
    log = get_logger("cli")
    log.info("starting MCP server", transport=args.transport)
...
async def _cmd_search(args: argparse.Namespace) -> None:
    ...
    try:
        ...
        output = [{"position": r.position, "title": r.title, "url": r.url, "snippet": r.snippet, "backend": r.backend} for r in results]
...
async def _cmd_fetch(args: argparse.Namespace) -> None:
    ...
    try:
        result = await fetcher.fetch(url=args.url, extract=args.extract, output_format=args.format, max_chars=args.max_chars, bypass_cache=args.no_cache)
        if args.raw:
...
async def _cmd_crawl(args: argparse.Namespace) -> None:
    ...
    try:
        result = await crawler.crawl(seed_url=args.url, max_depth=args.depth, max_pages=args.pages, same_domain_only=not args.allow_external, extract=not args.no_extract)
        _print_json(result.to_dict())
    finally:
        await fetcher.close()
        await robots.close()
...
async def _cmd_sitemap(args: argparse.Namespace) -> None:
    ...
    try:
        if args.discover:
            ...
            _print_json({"seed": args.url, "sitemaps_discovered": sitemaps, "url_count": len(all_urls), "urls": [{"url": u.url, "lastmod": u.lastmod.isoformat() if u.lastmod else None} for u in all_urls], "errors": all_errors})
        else:
            result = await parser.fetch_sitemap(args.url, recursive=not args.no_recursive)
            _print_json({"source": result.source_url, "is_index": result.is_index, "url_count": result.url_count, "sub_sitemaps": result.sub_sitemaps, "urls": [{"url": u.url, "lastmod": u.lastmod.isoformat() if u.lastmod else None} for u in result.urls], "errors": result.errors})
    finally:
        await parser.close()
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="webscout-mcp", description="A smart web search & fetch MCP server with caching and content extraction.")
    ...
    serve_parser.add_argument("--transport", choices=["stdio", "sse"], default="stdio", help="MCP transport protocol (default: stdio)")
    ...
    fetch_parser.add_argument("--no-extract", dest="extract", action="store_false", help="Don't extract main article content")
    fetch_parser.add_argument("--format", choices=["markdown", "text", "html"], default="markdown", help="Output format when extracting")
